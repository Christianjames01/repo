...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if ($stmt->execute()) $message = "Application status updated successfully!";
    else $error = "Error updating status: ".$stmt->error;
}
...
$query = "SELECT ja.*,j.job_title,j.company_name,j.company_logo,j.job_type,j.location,
           ...
           WHERE 1=1";
if ($search) $query .= " AND (ja.applicant_name LIKE '%".$conn->real_escape_string($search)."%' OR j.job_title LIKE '%".$conn->real_escape_string($search)."%' OR j.company_name LIKE '%".$conn->real_escape_string($search)."%')";
if ($job_filter) $query .= " AND ja.job_id=".intval($job_filter);
if ($status_filter) $query .= " AND ja.status='".$conn->real_escape_string($status_filter)."'";
...
$stats = $conn->query("SELECT COUNT(*) as total,
    ...
    FROM tbl_job_applications")->fetch_assoc();
...
<div class="jb-hero">
    ...
    <div class="jb-hero__inner">
        <div class="jb-hero__left">
            <div class="jb-hero__icon" style="background:linear-gradient(135deg,#0369a1,var(--db-sky));"><i class="fas fa-file-alt"></i></div>
            <div>
                <div class="jb-hero__title">Job Applications</div>
                <div class="jb-hero__sub">Review and manage all submitted applications</div>
            </div>
        </div>
        <div class="jb-hero__actions">
            <a href="manage-jobs.php" class="db-btn db-btn--ghost-white"><i class="fas fa-briefcase"></i> Manage Jobs</a>
...
<div class="db-stats-row">
    <a href="?status=" class="db-stat-card <?php echo !$status_filter?'active':''; ?>">
        <div class="db-stat-card__icon db-stat-card__icon--teal"><i class="fas fa-file-alt"></i></div>
    ...
    <a href="?status=Pending" class="db-stat-card <?php echo $status_filter==='Pending'?'active':''; ?>">
    ...
    <a href="?status=Reviewed" class="db-stat-card <?php echo $status_filter==='Reviewed'?'active':''; ?>">
        <div class="db-stat-card__icon db-stat-card__icon--sky"><i class="fas fa-eye"></i></div>
        <div><div class="db-stat-card__num" style="color:var(--db-sky)"><?php echo $stats['reviewed']; ?></div><div class="db-stat-card__label">Reviewed</div></div>
        <div class="db-stat-card__bar db-stat-card__bar--sky"></div>
    </a>
    <a href="?status=Shortlisted" class="db-stat-card <?php echo $status_filter==='Shortlisted'?'active':''; ?>">
        <div class="db-stat-card__icon db-stat-card__icon--violet"><i class="fas fa-star"></i></div>
        <div><div class="db-stat-card__num" style="color:var(--db-violet)"><?php echo $stats['shortlisted']; ?></div><div class="db-stat-card__label">Shortlisted</div></div>
        <div class="db-stat-card__bar db-stat-card__bar--violet"></div>
    </a>
    <a href="?status=Accepted" class="db-stat-card <?php echo $status_filter==='Accepted'?'active':''; ?>">
        <div class="db-stat-card__icon db-stat-card__icon--success"><i class="fas fa-check-circle"></i></div>
        <div><div class="db-stat-card__num" style="color:var(--db-success)"><?php echo $stats['accepted']; ?></div><div class="db-stat-card__label">Accepted</div></div>
        <div class="db-stat-card__bar db-stat-card__bar--success"></div>
    </a>
    <a href="?status=Rejected" class="db-stat-card <?php echo $status_filter==='Rejected'?'active':''; ?>">
...
<div class="db-panel">
    <div class="db-panel__header">
        ...
        <?php if ($search||$job_filter||$status_filter): ?>
        <a href="applications.php" class="db-btn db-btn--ghost db-btn--sm"><i class="fas fa-times"></i> Clear</a>
        <?php endif; ?>
    </div>
        ...
        <form method="GET">
            <div class="db-form-row">
                <div style="flex:2;min-width:180px;">
                    ...
                    <input type="text" name="search" class="db-input" style="width:100%;" placeholder="Search applicants or jobs…" value="<?php echo htmlspecialchars($search); ?>">
                </div>
                <div style="flex:2;min-width:160px;">
                    <label class="db-filter-label">Job</label>
                    <select name="job_id" class="db-select" style="width:100%;">
                        <option value="">All Jobs</option>
                        <?php while($j=$jobs->fetch_assoc()): ?>
                        <option value="<?php echo $j['job_id']; ?>" <?php echo $job_filter==$j['job_id']?'selected':''; ?>><?php echo htmlspecialchars($j['job_title'].' – '.$j['company_name']); ?></option>
                        <?php endwhile; ?>
                    </select>
                </div>
                <div style="flex:1;min-width:130px;">
                    ...
                    <select name="status" class="db-select" style="width:100%;">
                        ...
                        <?php foreach(['Pending','Reviewed','Shortlisted','Accepted','Rejected'] as $s): ?>
...
<div class="db-panel">
    <div class="db-panel__header">
        <div class="db-panel__title">
            ...
            <h2><?php echo $status_filter ? htmlspecialchars($status_filter).' Applications' : 'All Applications'; ?></h2>
            <span class="db-badge db-badge--sky"><?php echo $applications?$applications->num_rows:0; ?></span>
        </div>
        <span class="db-text-sm"><i class="fas fa-info-circle"></i> Click <i class="fas fa-eye"></i> to review</span>
    </div>
    <div class="db-panel__body" style="padding:16px 22px;">
    <?php if ($applications && $applications->num_rows > 0):
        $sc=['Pending'=>'amber','Reviewed'=>'sky','Shortlisted'=>'violet','Accepted'=>'success','Rejected'=>'rose','Withdrawn'=>'muted'];
        $si=['Pending'=>'clock','Reviewed'=>'eye','Shortlisted'=>'star','Accepted'=>'check-circle','Rejected'=>'times-circle','Withdrawn'=>'ban'];
    ...
    <div class="db-app-card">
        ...
        <img src="../../../uploads/company_logos/<?php echo htmlspecialchars($app['company_logo']); ?>" class="db-app-card__logo" alt="">
        <?php else: ?>
        <div class="db-app-card__logo-placeholder"><i class="fas fa-building"></i></div>
        <?php endif; ?>

        <div class="db-app-card__info" style="flex:1.2;">
            <div class="db-app-card__name"><?php echo htmlspecialchars($app['applicant_name']); ?></div>
            <div class="db-app-card__meta"><i class="fas fa-envelope" style="width:12px;"></i> <?php echo htmlspecialchars($app['email']??$app['resident_email']??'N/A'); ?></div>
            <div class="db-app-card__meta"><i class="fas fa-phone" style="width:12px;"></i> <?php echo htmlspecialchars($app['phone']??$app['contact_number']??'N/A'); ?></div>
        </div>

        <div class="db-app-card__info" style="flex:1.4;">
            <div class="db-app-card__job"><?php echo htmlspecialchars($app['job_title']); ?></div>
            <div class="db-app-card__company"><?php echo htmlspecialchars($app['company_name']); ?></div>
            <div style="margin-top:4px;display:flex;gap:5px;">
                <span class="db-badge db-badge--muted"><i class="fas fa-briefcase"></i> <?php echo htmlspecialchars($app['job_type']); ?></span>
                <span class="db-badge db-badge--muted"><i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($app['location']); ?></span>
            </div>
        </div>

        <div class="db-app-card__date">
            <div class="db-text-sm">Applied</div>
            <strong style="font-size:12px;"><?php echo date('M d, Y',strtotime($app['application_date'])); ?></strong>
        </div>

        <div style="text-align:center;min-width:90px;">
            <span class="db-badge db-badge--<?php echo $cls; ?>"><i class="fas fa-<?php echo $ico; ?>"></i> <?php echo $st; ?></span>
        </div>

        <div class="db-app-card__actions">
            <button class="db-btn db-btn--sky db-btn--sm" onclick='openViewApp(<?php echo json_encode($app); ?>)'>
                <i class="fas fa-eye"></i> Review
            </button>
        </div>
    </div>
    <?php endwhile; else: ?>
    <div class="db-empty"><i class="fas fa-inbox"></i><p>No applications found</p></div>
    <?php endif; ?>
    </div>
</div>
...
<div id="viewAppModal" class="db-modal">
    <div class="db-modal__box db-modal__box--lg">
        <div class="db-modal__header db-modal__header--sky">
            ...
            <button class="db-modal__close" onclick="closeModal('viewAppModal')">×</button>
        </div>
        <div class="db-modal__body">
            <!-- Logo -->
            <div id="va_logo_wrap" style="text-align:center;margin-bottom:14px;"></div>
            <!-- Info grid -->
            <div class="db-form-row-2" style="gap:0;margin-bottom:16px;" id="va_info_grid"></div>
            <!-- Position -->
            <div style="margin-bottom:16px;" id="va_position"></div>
            <!-- Cover letter -->
            <div class="db-form-group" id="va_cover_wrap">
                <label>Cover Letter</label>
                <div id="va_cover" style="background:var(--db-surf2);border:1px solid var(--db-border);border-radius:var(--db-radius-sm);padding:14px;font-size:13px;line-height:1.6;white-space:pre-line;min-height:80px;"></div>
            </div>
            <!-- Resume -->
            <div id="va_resume_wrap"></div>
            <hr style="border:none;border-top:1px solid var(--db-border);margin:16px 0;">
            <!-- Update form -->
            <form method="POST">
                <input type="hidden" name="application_id" id="va_app_id">
                <div class="db-form-row-2">
                    <div class="db-form-group">
                        <label>Update Status</label>
                        <select name="status" id="va_status" class="db-select">
                            <?php foreach(['Pending','Reviewed','Shortlisted','Accepted','Rejected'] as $s): ?>
                ...
                <div class="db-form-group">
                    ...
                    <textarea name="notes" id="va_notes" class="db-input db-textarea" rows="3" placeholder="Add notes about this application…"></textarea>
                </div>
                <div id="va_existing_notes"></div>
                <div class="db-modal__footer">
                    <button type="button" class="db-btn db-btn--ghost" onclick="closeModal('viewAppModal')">Close</button>
                    <button type="submit" name="update_status" class="db-btn db-btn--primary"><i class="fas fa-save"></i> Update Status &amp; Notes</button>
                </div>
            </form>
        </div>
    </div>
...
function openViewApp(a){
    document.getElementById('va_app_id').value=a.application_id;
    document.getElementById('va_status').value=a.status||'Pending';
    document.getElementById('va_notes').value='';

    const lw=document.getElementById('va_logo_wrap');
    if(a.company_logo) lw.innerHTML=`<img src="../../../uploads/company_logos/${a.company_logo}" style="max-height:72px;border-radius:10px;border:1px solid var(--db-border);padding:6px;">`;
    else lw.innerHTML='';
    ...
    document.getElementById('va_info_grid').innerHTML=
        row('Applicant Name',a.applicant_name)+
        row('Email',a.email||a.resident_email)+
        row('Phone',a.phone||a.contact_number)+
        row('Application Date',new Date(a.application_date).toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}));

    document.getElementById('va_position').innerHTML=`<div class="db-form-group"><label>Job Position</label><div style="font-size:14px;font-weight:700;">${a.job_title}</div><div class="db-text-sm">${a.company_name}</div><div style="margin-top:6px;display:flex;gap:6px;"><span class="db-badge db-badge--sky">${a.job_type}</span><span class="db-badge db-badge--muted">${a.location}</span></div></div>`;

    document.getElementById('va_cover').textContent=a.cover_letter||'N/A';

    const rw=document.getElementById('va_resume_wrap');
    if(a.resume_file) rw.innerHTML=`<div class="db-form-group"><label>Resume</label><a href="../../../uploads/resumes/${a.resume_file}" class="db-btn db-btn--teal db-btn--sm" target="_blank"><i class="fas fa-download"></i> Download Resume</a></div>`;
    else rw.innerHTML='';

    const en=document.getElementById('va_existing_notes');
    if(a.notes) en.innerHTML=`<div class="db-notice db-notice--amber" style="margin-bottom:10px;"><i class="fas fa-sticky-note" style="flex-shrink:0;"></i><div><strong>Current Notes:</strong><br>${a.notes}</div></div>`;
    else en.innerHTML='';
